...
const createAutocompleteList = (cursorPosition: any) => {
  ...
  const lineNum = list2D.findIndex(
    (line) => line.join(" ") === cursorPosition[2]
  );
  ...
  const inputSubList = entry.commands.map(
    (cmd: any) => cmd[subcommand]?.subcommand
  );
  if (subcommand) {
    let command = entry.commands.find(
      (cmd: any) => cmd[subcommand]?.subcommand === line[subcommand]
    );

    if (command) {
      if (selectedLine.length - 1 == subcommand) return false;
      ...
      if (selectedLine.length - 1 == subcommand - 1) return false;
      return {
        word: selectedWord,
        ranges: inputSubList,
      };
...
const mergeA = (cursor: any) => {
  ...
  const results1: any[] = [];
  const arrays =
    Array.isArray(data.ranges[0]) || typeof data.ranges[0] === "object"
      ? data.ranges
      : [data.ranges];
  arrays.forEach((range: any) => {
    ...
    if (Array.isArray(range)) {
      ...
      if (type != "$#$obj") {
        return results1.push(rest);
      } else {
        const foundConfig = rest.find((item: any) =>
          item.blocks?.includes(data.line[data.line.length - 2])
        );
